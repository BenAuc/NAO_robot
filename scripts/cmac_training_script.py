...
def get_L2_neuron_position(input_data):
    ...
    for i_channel in range(cmac_nb_inputs):
        ...
        quantized_ip = int(input_normalization(input_data)[i_channel] * cmac_res)
...
cmac_field_size = 3 # receptive field size: set to 3 or 5 depending on the task, see tutorial description
...
cmac_rf = [[0, 0], [1, 1], [2, 2]] 
cmac_weight_table = np.random.uniform(-0.25, 0.25, (cmac_res, cmac_res, cmac_nb_outputs)) # Not all entries correspond to a neuron, depends on cmac_nb_neurons
